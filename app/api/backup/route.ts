...
export async function POST(request: NextRequest) {
  try {
    ...
    if (type === 'database') {
      ...
      const databaseBackupDir = path.join(BACKUP_ROOT, 'Database');
      const backupPath = path.join(databaseBackupDir, backupFile);

      // Tạo thư mục backup trên máy chạy Next.js.
      // SQL Server cũng phải có quyền ghi vào thư mục này.
      const { mkdir, access, constants } = await import('fs/promises');
      await mkdir(databaseBackupDir, { recursive: true });
      await access(databaseBackupDir, constants.W_OK);

      // Escape ký tự ' để tạo SQL string literal an toàn.
      const escapeSqlString = (value: string) => value.replace(/'/g, "''");
      const safeBackupPath = escapeSqlString(backupPath);
      ...
      try {
        ...
        console.error('SQL Server backup error:', {
        ...
        const details = [
          ...
          sqlError?.precedingErrors?.map((e: any) => e?.message).join(' | '),
        ].filter(Boolean).join(' | ');

        return NextResponse.json(
          {
            success: false,
            message: `Backup database thất bại: ${details || 'SQL Server không cung cấp thông tin lỗi chi tiết'}`,
            error: {
              code: sqlError?.code,
              number: sqlError?.number,
              state: sqlError?.state,
    ...
    console.error('Lỗi khi thực hiện backup:', {
    ...
    return NextResponse.json(
      {
        success: false,
        message: details || 'Lỗi khi thực hiện backup',
      },
      { status: 500 }
    );
  }
