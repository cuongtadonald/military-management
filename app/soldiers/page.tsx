...
import { App, Avatar, Button, Card, Col, Input, Layout, Modal, Row, Select, Space, Table, Tabs, Tooltip, Typography, TreeSelect } from "antd"
import type { ColumnsType } from "antd/es/table"
import { DeleteOutlined, EditOutlined, EyeOutlined, ExclamationCircleFilled, FileExcelOutlined, PlusOutlined, SearchOutlined, UploadOutlined, SendOutlined, CalendarOutlined } from "@ant-design/icons"
...
export default function SoldierListPage() {
  ...
  const currentData = useMemo(() => {
    if (activeTab === "all") return [...activeData, ...dischargedData]
    if (activeTab === "discharged") return dischargedData
    return activeData
  }, [activeTab, activeData, dischargedData])
  ...
  const columns: ColumnsType<SoldierData> = [
    ...
    {
      title: "Đơn vị", key: "Unit", width: 150,
      render: (_, record) => {
        const parts = (record.UnitFullPath || "").split(',')
        const current = parts.length > 0 ? parts[parts.length - 1] : record.UnitName
        return <span style={{ fontSize: 13 }}>{current}</span>
      },
    },
    {
        ...
        <Row gutter={[12, 12]} align="middle">
          ...
          <Col>
            <TreeSelect
              ...
              loading={unitTreeLoading} style={{ width: 200, borderRadius: 8 }}
